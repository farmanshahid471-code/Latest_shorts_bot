...
from .config import CLIP_DURATION_SEC as DEFAULT_CLIP_SECONDS
from .config import clamp_clip_duration
...
def create_app(testing: bool = False) -> Flask:
    ...
    def api_run_once():
        ...
        _spawn_job(
            "run-once",
            lambda: ShortsBotScheduler().run_single_cycle(preflight_oauth=True),
        )
...
def _render_page(msg: str = "", msg_type: str = "ok",
    ...
    <div class="card" style="margin-top:16px;">
      ...
      <form action="/api/platform-settings/save" method="POST">
        ...
        <table style="width:100%;font-size:13px;border-collapse:collapse;margin-top:6px;">
          <tr><td style="padding:4px 0;width:38%;">Clip length (seconds)</td>
              <td><input type="number" name="{platform}_clip_seconds" value="{_esc(pset["clip_seconds"])}"
                    min="5" max="180" step="1" placeholder="{_esc(pset["clip_placeholder"])}" style="width:100%;">
                  <div class="hint">{clip_hint}</div></td></tr>
